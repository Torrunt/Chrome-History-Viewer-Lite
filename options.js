function onLoaded()
{
    ...
    if (lines.value == 0)
        linesValue.textContent = "No Limit";
    lines.addEventListener('input', function (event) { linesValue.textContent = event.target.value == 0 ? "No Limit" : event.target.value; }, false);
