...
function loadContent()
{
    ...
    if (settings.menuTop == true)
        content = document.getElementById("content2");
    if (settings.boldFont == true)
        content.className += " bold";
    if (settings.showTime == true)
        content.className += " timeEnabled";
    if (settings.showSearch == true)
        document.getElementById("searchQ").focus(); else document.getElementById("searchQ").style.display = "none";
...
function searchFor(string)
{
    ...
    filterTimeOut = setTimeout(loadContent, 250);
}

function showAllHistory()
{
    var str = "chrome://history/";
    if (filterStringsArray != null)
        str += "#q=" + filterString;
...
function ShowEntry(item)
{
    ...
    if (filterStringsArray != null)
        item.title = item.title.multiReplace(filterStringsArray);

    html += "<div";
    if (settings.numLines != 0 && !isNaN(settings.numLines))
        html += " class=\"maxh" + settings.numLines + "\"";
    html += "> " + item.title + "</div>";
    ...
    if (settings.showTime)
    {
        ...
        if (hoursDifference < 1 && minutesDifference < 1 && secondsDifference < 60)
            timeTextz = '<b>' + secondsDifference + 's</b> ago';
        else if (hoursDifference < 1)
            timeTextz = '<b>' + minutesDifference + ' min</b> ago';
        else if (hoursDifference < 4)
            timeTextz = '<b>' + hoursDifference + 'h ' + minutesDifference + 'min</b>';
        else if (hoursDifference < 24)
            timeTextz = '<b>' + hoursDifference + 'h</b> ago';
        else
        {
